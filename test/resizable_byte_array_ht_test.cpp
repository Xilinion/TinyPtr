...
void concurrent_insert(ResizableByteArrayChainedHT& ht,
...
void concurrent_query(ResizableByteArrayChainedHT& ht,
                      const vector<pair<uint64_t, uint64_t>>& data, int start,
                      int end) {
    uint64_t handle = ht.GetHandle();
    for (int i = start; i < end; ++i) {
...
TEST(ResizableByteArrayChainedHT_TESTSUITE, ParallelInsertQuery) {
    srand(233);
