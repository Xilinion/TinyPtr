...
// Ensure ConcurrentByteArrayChainedHT is defined in concurrent_skulker_ht.h
...
void concurrent_insert(ConcurrentByteArrayChainedHT& ht,
...
void concurrent_query(ConcurrentByteArrayChainedHT& ht,
                      const vector<pair<uint64_t, uint64_t>>& data, int start,
                      int end) {
    for (int i = start; i < end; ++i) {
        uint64_t val = 0;
...
TEST(ConcurrentByteArrayChainedHT_TESTSUITE, ParallelInsertQuery) {
    srand(233);
