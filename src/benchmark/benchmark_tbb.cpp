...
const BenchmarkObjectType BenchmarkTBB::TYPE =
...
void BenchmarkTBB::YCSBFill(std::vector<uint64_t>& keys, int num_threads) {
    ...
    for (int i = 0; i < num_threads; ++i) {
        ...
        threads.emplace_back(
            [this, &keys, start_index, end_index, i]() {
...
void BenchmarkTBB::YCSBRun(std::vector<std::pair<uint64_t, uint64_t>>& ops,
    ...
    for (int i = 0; i < num_threads; ++i) {
        ...
        threads.emplace_back(
            [this, &ops, start_index, end_index, i]() {
                for (size_t j = start_index; j < end_index; ++j) {
                    if (ops[j].first == 1) {
...
std::vector<std::tuple<uint64_t, double, uint64_t>> BenchmarkTBB::YCSBRunWithLatencyRecording(
...
std::vector<std::tuple<uint64_t, double, uint64_t>> BenchmarkTBB::ConcurrentRunWithLatencyRecording(
